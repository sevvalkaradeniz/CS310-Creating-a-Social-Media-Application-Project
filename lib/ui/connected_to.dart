...
class ConnectedToPage extends StatefulWidget {
  const ConnectedToPage({
    Key? key,
    required this.user,
  }) : super(key: key);
  final AppUser user;
  @override
  State<ConnectedToPage> createState() => _ConnectedToPageState();
}

class _ConnectedToPageState extends State<ConnectedToPage> {
  @override
  Widget build(BuildContext context) {
    AppUser user = Provider.of<UserProvider>(context).user!;
    DB db = DB();
    Future<dynamic> getconnecteds() async {
      final connecteds = await db.getUsersConnectedList(widget.user);
    ...
    return Scaffold(
      ...
      body: FutureBuilder<dynamic>(
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          ...
          if (!snapshot.hasData) {
            return const Center(
              child: Text('You are not connected to anyone'),
            );
          }
          if (snapshot.data!['connecteds'] != null &&
              snapshot.data!['connecteds'].isEmpty) {
            return const Center(
              child: Text('You are not connected to anyone'),
            );
          }
          return Container(
            ...
            child: ListView.builder(
              itemCount: snapshot.data!['connecteds'].length,
              itemBuilder: (context, index) {
                return userComponent(
                  context,
                  setState,
                  user: snapshot.data!['connecteds'][index],
